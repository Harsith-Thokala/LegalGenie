...
export default function DocumentsPage() {
  ...
  const handleDownloadPDF = async (document: any) => {
    try {
      console.log('Starting PDF generation for document:', document.title)
      ...
      doc.text(document.title || 'Untitled Document', margin, margin + 10)

      // Add metadata
      doc.setFontSize(10)
      doc.setFont(undefined, "normal")
      doc.text(`Type: ${document.type || 'Legal Document'}`, margin, margin + 25)
      
      // Handle date formatting safely
      const createdDate = document.created_at ? new Date(document.created_at).toLocaleDateString() : 'Unknown'
      const updatedDate = document.updated_at ? new Date(document.updated_at).toLocaleDateString() : 'Unknown'
      
      doc.text(`Created: ${createdDate}`, margin, margin + 35)
      doc.text(`Modified: ${updatedDate}`, margin, margin + 45)
      ...
      const fileName = (document.title || 'document').replace(/[^a-z0-9]/gi, '_').toLowerCase()
      ...
      alert('PDF generation failed. Downloading as TXT instead.')
